                            ...
                            @haspermission('modify','inventory')
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900">
                                        {{ number_format($record->total_products) }}
                                    </div>
                                </td>
                    ...
                    <div class="flex items-center justify-between gap-10">
                        ...
                        @else
                            <div class="mt-4 bg-amber-50 border border-amber-200 rounded-lg p-4">
                                ...
                                <p class="text-xs text-amber-600 mt-1">⚠️ Changing the date will update this record's
                                    date</p>
                            </div>
                        @endif
...
<div class="flex justify-between pt-6 border-t border-gray-200">
    ...
    {{-- Credit Customer Info Modal --}}
@if ($showCreditCustomerModal)
    <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-[60]">
        <div class="bg-white rounded-lg p-6 w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto">
            <div class="flex justify-between items-center mb-6">
                <h3 class="text-lg font-semibold text-gray-900">Credit Customer Information</h3>
                <button wire:click="closeCreditCustomerModal" class="text-gray-400 hover:text-gray-600">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="mb-4 bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                <p class="text-sm text-yellow-800">
                    ...
                    You have items sold on credit. Please provide customer information for tracking.
                </p>
            </div>

            <div class="space-y-6">
                @foreach($pendingCreditProducts as $productId => $creditInfo)
                    <div class="border border-gray-200 rounded-lg p-4 bg-gray-50">
                        <div class="mb-3">
                            <h4 class="font-medium text-gray-900 uppercase">{{ $creditInfo['name'] }}</h4>
                            <p class="text-sm text-gray-600">
                                Units: {{ $creditInfo['units'] }} • 
                                Amount: GH₵ {{ number_format($creditInfo['amount'], 2) }}
                            </p>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input 
                                    type="text" 
                                    wire:model="creditCustomerData.{{ $productId }}.name"
                            ...
                            <div>
                                ...
                                <input 
                                    type="tel" 
                                    wire:model="creditCustomerData.{{ $productId }}.phone"
            ...
            <div class="flex justify-end gap-3 mt-6 pt-6 border-t border-gray-200">
                <button 
                    wire:click="closeCreditCustomerModal"
                    class="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50">
                    Cancel
                </button>
                <button 
                    wire:click="proceedWithInventory"
