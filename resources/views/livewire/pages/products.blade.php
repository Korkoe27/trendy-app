<div class="space-y-6">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($products as $product)
                        ...
                        <tr class="hover:bg-gray-50" wire:key="product-row-{{ $product->id }}">
                            ...
                            {{-- Cost/Sell/Margin --}}
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-base text-gray-900">GH₵ {{ number_format($product->stocks->cost_price ?? 0, 2) }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-base text-gray-900">GH₵ {{ number_format($product->selling_price, 2) }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
    ...
    @if($showModal && $product)
        <div class="fixed inset-0 bg-black/50 bg-opacity-50 flex items-center justify-center z-50" aria-labelledby="modal-title" role="dialog" aria-modal="true">
                ...
                <div class="bg-white rounded-lg p-6 w-full max-w-6xl mx-4 max-h-[90vh] overflow-y-auto">
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="space-y-4">
                                ...
                                <div>
                                    ...
                                    <p class="text-base font-semibold text-gray-900">GH₵ {{ number_format($product->selling_price, 2) }}</p>
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <span class="inline-block px-2 py-1 text-sm font-medium rounded-full {{ $product->is_active ? 'text-green-600 bg-green-100' : 'text-gray-600 bg-gray-100' }}">
                                        {{ $product->is_active ? 'Active' : 'Inactive' }}
                                    </span>
                                </div>
                            </div>
                            ...
                            <div class="space-y-4">
                                ...
                                <div>
                                    ...
                                    <span class="inline-block px-2 py-1 text-sm font-medium rounded-full {{ $stockStatus['color'] }}">
                                        {{ $stockStatus['text'] }}
                                    </span>
                                </div>
                                ...
                                @if($latestStock)
                                    <div>
                                        ...
                                        <p class="text-base text-gray-900">GH₵ {{ number_format($latestStock->cost_price, 2) }}</p>
                                    </div>
                                    <div>
                                        <label class="block text-sm font-medium text-gray-600">Cost Margin</label>
                                        <p class="text-base font-medium {{ $latestStock->cost_margin > 50 ? 'text-green-600' : ($latestStock->cost_margin > 25 ? 'text-yellow-600' : 'text-red-600') }}">
                                            {{ $latestStock->cost_margin ?? 0 }}%
                                        </p>
                                    </div>
                                    <div>
                                        <label class="block text-sm font-medium text-gray-600">Last Updated</label>
                                        <p class="text-sm text-gray-700">{{ $latestStock->created_at->format('M d, Y h:i A') }}</p>
                                    </div>
                                @endif
                            </div>
                        </div>
                        ...
                        @if($stockHistory && $stockHistory->count() > 0)
                            <div class="mt-6">
                                <h4 class="text-md font-semibold text-gray-800 border-b pb-2 mb-4">Recent Stock History</h4>
                                <div class="overflow-x-auto">
                                    <table class="min-w-full divide-y divide-gray-200">
                                        <thead class="bg-gray-50">
                                            <tr>
                                        ...
                                        <tbody class="bg-white divide-y divide-gray-200">
                                            @foreach($stockHistory as $stock)
                                                <tr>
                                                    <td class="px-4 py-2 text-sm text-gray-900">{{ $stock->created_at->format('M d, Y') }}</td>
                                                    <td class="px-4 py-2 text-sm font-medium text-gray-900">{{ $stock->total_units }}</td>
                                                    <td class="px-4 py-2 text-sm text-gray-900">GH₵ {{ number_format($stock->cost_price, 2) }}</td>
                                                    <td class="px-4 py-2 text-sm font-medium {{ $stock->cost_margin > 50 ? 'text-green-600' : ($stock->cost_margin > 25 ? 'text-yellow-600' : 'text-red-600') }}">
                                                        {{ $stock->cost_margin ?? 0 }}%
                                                    </td>
                                                </tr>
                                            @endforeach
                                ...
                                <div class="w-full">
                                    ...
                                    <div class="space-y-4">
                                        ...
                                        <div>
                                            ...
                                            <input type="text" wire:model="name" id="name" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 capitalize focus:ring-blue-500 focus:border-transparent">
                                        ...
                                        <div>
                                            ...
                                            <select wire:model="category_id" id="category_id" class="w-full px-3 py-2 border border-gray-300 rounded-lg capitalize focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                                        ...
                                        <div>
                                            ...
                                            <input type="text" wire:model="barcode" id="barcode" class="mt-1 block w-full border border-gray-300 rounded-md capitalize shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                                            @error('barcode') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                                        </div>

                                        <!-- Selling Price -->
                                        <div>
                                            <label for="selling_price" class="block text-sm font-medium text-gray-700">Selling Price (GH₵)</label>
                                            <input type="number" min="0" step="0.01" wire:model="selling_price" id="selling_price" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 capitalize px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                                            @error('selling_price') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                                        </div>

                                        <!-- Stock Limit -->
                                        <div>
                                            <label for="stock_limit" class="block text-sm font-medium text-gray-700">Stock Limit (Optional)</label>
                                            <input type="number" min="0" wire:model="stock_limit" id="stock_limit" class="mt-1 block w-full border border-gray-300 rounded-md capitalize shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                                            @error('stock_limit') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                                        </div>
