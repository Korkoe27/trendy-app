...
@if ($showCreateModal)
    <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50" wire:click.self="closeCreateModal">
        <div class="bg-white rounded-lg p-6 w-full max-w-6xl mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <form wire:submit="saveNewProducts" class="space-y-6">
                <div class="mb-6">
                    ...
                    <div class="border border-gray-200 rounded-lg overflow-hidden">
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-gray-50">
                                    <tr>
                                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Product Name <span class="text-red-600">*</span></th>
                                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Category <span class="text-red-600">*</span></th>
                                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">SKU</th>
                                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Barcode<span class="text-red-600">*</span></th>
                                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Stock Limit<span class="text-red-600">*</span></th>
                                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Selling Price (GH₵)<span class="text-red-600">*</span></th>
                                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Units/Box</th>
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @forelse($newProducts as $index => $product)
                                        <tr class="hover:bg-gray-50">
                                            <td class="px-4 py-3">
                                                <input type="text" wire:model="newProducts.{{ $index }}.name" 
                                                    placeholder="Small Club Beer"
                                                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 text-sm" required />
                                                @error('newProducts.' . $index . '.name')
                                                    <div class="text-red-500 text-xs mt-1">{{ $message }}</div>
                                                @enderror
                                            </td>
                                            <td class="px-4 py-3">
                                                <select wire:model="newProducts.{{ $index }}.category_id"
                                                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 text-sm" required>
                                                    <option value="">Select Category</option>
                                                    @foreach($categories as $category)
                                            ...
                                            <td class="px-4 py-3">
                                                <input type="text" wire:model="newProducts.{{ $index }}.sku" placeholder="SKU123"
                                            ...
                                            <td class="px-4 py-3">
                                                <input type="text" 
                                                oninput="this.value = this.value.replace(/[^0-9]/g, '');"
                                                wire:model="newProducts.{{ $index }}.barcode" placeholder="1234567890"
                                                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 text-sm" />
                                            ...
                                            <td class="px-4 py-3">
                                                <input type="text"
                                                oninput="this.value = this.value.replace(/[^0-9]/g, '');"
                                                wire:model="newProducts.{{ $index }}.stock_limit" placeholder="100"
                                                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 text-sm" min="0" />
                                            ...
                                            <td class="px-4 py-3">
                                                <input type="text" 
                                                oninput="this.value = this.value.replace(/[^0-9]/g, '');"
                                                wire:model="newProducts.{{ $index }}.selling_price" step="0.01" placeholder="0.00"
                                                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 text-sm" min="0" required />
                                                @error('newProducts.' . $index . '.selling_price')
                                                    <div class="text-red-500 text-xs mt-1">{{ $message }}</div>
                                                @enderror
                                            </td>
                                            <td class="px-4 py-3">
                                                <input type="text" wire:model="newProducts.{{ $index }}.units_per_box" step="0.01" 
                                                oninput="this.value = this.value.replace(/[^0-9]/g, '');"
                                                placeholder="24"
                                                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 text-sm" min="0" />
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="8" class="px-4 py-3 text-center text-gray-500">No products added yet</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
