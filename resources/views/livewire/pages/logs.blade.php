...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-lg border border-gray-200 p-4 md:p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mt-6">
            <div class="relative">
                ...
                <input type="text" placeholder="Search activities..." wire:model.live.debounce.300ms="searchTerm"
                    class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm" />
            </div>
            <select wire:model.live="selectedActionType"
                class="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm">
    ...
    <div class="bg-white rounded-lg border border-gray-200 overflow-hidden">
        {{-- Table Controls --}}
        <div
            class="px-6 py-4 border-b border-gray-200 flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-3 sm:space-y-0">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors duration-150">
                            <td class="px-4 md:px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <span
                                        class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $this->getActionColor($log->action_type) }}">
                                        ...
                                        @elseif($icon === 'shopping-cart')
                                            <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                    d="M3 3h2l.4 2M7 13h10l4-8H5.4m0 0L7 13m0 0l-2.5 5M7 13l2.5 5m0 0L12 21m-2.5-3H17" />
                                            </svg>
                                        @elseif($icon === 'database')
                                            <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                    d="M4 7v10c0 2 6 2 8 2s8 0 8-2V7M4 7c0 2 6 2 8 2s8 0 8-2M4 7c0-2 6-2 8-2s8 0 8 2" />
                                            </svg>
                                        @elseif($icon === 'users')
                                            <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                            ...
                            <td class="px-4 md:px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div
                                        class="w-8 h-8 bg-gray-300 rounded-full flex items-center justify-center mr-3 flex-shrink-0">
                                    ...
                                    <div class="min-w-0">
                                        <div class="text-sm capitalize font-medium text-gray-900 truncate">
                                            {{ $log->user->name ?? 'System' }}</div>
                                        <div class="text-xs text-gray-500 truncate">
                                            {{ $log->user->role ?? 'Automated' }}</div>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-4 md:px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900">
                                    <div class="font-medium">{{ ucfirst(str_replace('_', ' ', $log->entity_type)) }}
                                    </div>
                                    @if ($log->entity_id)
                            ...
                            <td class="px-4 md:px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center text-sm text-gray-900">
                                    ...
                                    <div class="min-w-0">
                                        ...
                                        <div class="text-xs text-gray-500 truncate">
                                            {{ $log->created_at->format('h:i:s A') }}</div>
                                    </div>
                                </div>
                            </td>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        <div class="space-y-4">
                            <div>
                                ...
                                <div>
                                    <span
                                        class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium {{ $this->getActionColor($selectedLog->action_type) }}">
                                ...
                                <div class="flex items-center">
                                    ...
                                    <div>
                                        <div class="text-sm font-medium text-gray-900">
                                            {{ $selectedLog->user->name ?? 'System' }}</div>
                                        <div class="text-xs text-gray-500">
                                            {{ $selectedLog->user->email ?? 'system@automated' }}</div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="space-y-4">
                            ...
                            @if ($selectedLog->description)
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-2">Activity
                                        Summary</label>
                                    <div
                                        class="text-sm text-gray-900 bg-gray-50 border border-gray-200 rounded-md p-3">
                            ...
                            @if ($selectedLog->metadata)
                                <div>
                                    ...
                                    <div
                                        class="bg-gray-50 border border-gray-200 rounded-md p-3 text-sm text-gray-900 overflow-x-auto">
