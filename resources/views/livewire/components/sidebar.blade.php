<div class="relative">
    ...
    <!-- Sidebar -->
    <div class="fixed lg:relative inset-y-0 left-0 min-h-screen md:rounded-2xl z-50 w-72 bg-white shadow-lg transform transition-transform duration-300 ease-in-out {{ $isOpen ? 'translate-x-0' : '-translate-x-full' }} lg:translate-x-0">
        ...
        <!-- Navigation -->
        <nav class="flex-1 p-4 space-y-2 overflow-y-auto" style="max-height: calc(100vh - 180px);">
            <!-- Dashboard Link -->
            <a href="{{ route('dashboard') }}"
               wire:navigate 
               wire:click="setActiveItem('dashboard'); closeSidebar()"
            ...
            <a href="{{ route('inventory') }}" 
               wire:navigate
               wire:click="setActiveItem('inventory'); closeSidebar()"
            ...
            <a href="{{ route('products') }}" 
               wire:navigate
               wire:click="setActiveItem('products'); closeSidebar()"
            ...
            <a href="{{ route('stocks') }}" 
               wire:navigate
               wire:click="setActiveItem('stocks'); closeSidebar()"
            ...
            <!-- Logs Link -->
            <a href="{{ route('spending') }}" 
               wire:navigate
               wire:click="setActiveItem('spending'); closeSidebar()"
               class="flex items-center gap-3 px-4 py-2.5 text-sm font-medium rounded-lg transition-colors duration-200 {{ request()->routeIs('spending') ? 'bg-black text-white' : 'bg-white text-gray-700 hover:bg-gray-100 hover:text-gray-900' }}">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 shrink-0">
            ...
            <a href="{{ route('logs') }}" 
               wire:navigate
               wire:click="setActiveItem('logs'); closeSidebar()"
            ...
            <a href="{{ route('users') }}"
            wire:navigate
            wire:click="setActiveItem('users'); closeSidebar()"
            ...
            <a href="{{ route('analytics') }}" 
               wire:navigate
               wire:click="setActiveItem('analytics'); closeSidebar()"
               class="flex items-center gap-3 px-4 py-2.5 text-sm font-medium rounded-lg transition-colors duration-200 {{ request()->routeIs('analytics') ? 'bg-black text-white' : 'bg-white text-gray-700 hover:bg-gray-100 hover:text-gray-900' }}">
            ...
            <a href="{{ route('settings') }}" 
                wire:navigate
                wire:click="setActiveItem('settings'); closeSidebar()"
    ...
    <button wire:click="toggleSidebar" 
            class="fixed top-4 left-4 z-50 p-2 bg-white rounded-md shadow-lg lg:hidden {{ $isOpen ? 'hidden' : '' }}">
